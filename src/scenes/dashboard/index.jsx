...
const Dashboard = () => {
  ...
  useEffect(() => {
    fetch("https://brainly-back.onrender.com/doctores")
  ...
  useEffect(() => {
    fetch("https://brainly-back.onrender.com/pacientes")
      .then((response) => response.json())
      .then((jsonData) => {
        ...
        const numFemales = transformedDatas.filter(
          (user) => user.genero === "femenino"
        ).length;
        ...
        const numMales = transformedDatas.filter(
          ...
          {fiveLastUsers.map((user, i) => (
            ...
            >
              <Box>
                ...
                <Typography color={colors.grey[100]}>{user.usuario}</Typography>
              </Box>
